...
<!-- BEGIN panel-heading -->
        <div class="panel-heading" >
            ...
            <h4><span class="" style="color:#9c27b0;">Subcategorías</span> Registradas</h4>
        </div>
<!-- END panel-heading -->
<!-- BEGIN panel-body -->
        <div class="panel-body">
          ...
          <table id="datatables-default" class="table  m-0 ">
				<thead>
					<tr >
						...
						<th class="no-sort" style="width:10%; color:#888;">Opciones</th>
					</tr>
				</thead>
				<tbody>
          <!--Inicio Foreach-->
         @foreach ($subcategorias as $subcat)
					<tr style="color:#888;">
						...
						<td>{{ $subcat->idsubcategoria}}</td>
						<td>{{ $subcat->nombre}}</td>
						<td>{{ $subcat->categoria}}</td>
           @if ($subcat->estado=='Activo')
           <td><span class="label label" style="background:#5cb85c;">{{ $subcat->estado}}</span></td>
           @else
           <td><span class="label label" style="background:#8A8A8F;">{{ $subcat->estado}}</span></td>
           @endif
						<td class="btn-col" style="white-space: nowrap">
              <a href=""  data-target="#modal-edit-{{$subcat->idsubcategoria}}" data-toggle="modal"><button class="btn btn btn-xs" style="background:#00bcd4; color:#fff;font-weight: bold;"><i class="ti-pencil"></i></button></a>

               <a href="" data-target="#modal-delete-{{$subcat->idsubcategoria}}" data-toggle="modal" ><button class="btn btn-pink btn-xs" style="background:#ff5160; color:#fff;font-weight: bold;"
                 ><i class="ti-close"></i></button></a>
						</td>
					</tr>


          @include('almacen.subcategoria.edit',[$subcat->nombre,$subcat->categoria,$subcat,$categorias])
				  @include('almacen.subcategoria.modal')
          @endforeach
          <!--Fin Foreach-->
				</tbody>
			</table>
...
<!-- END panel-body -->
    ...
    @include('almacen.subcategoria.create')
    <a href=""  data-target="#modal-create" data-toggle="modal"><button class="btn btn-pink btn-sm pull-left m-l-5" >Nueva Subcategoría</button></a>
