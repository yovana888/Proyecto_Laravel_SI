...
role="dialog" tabindex="-1" id="modal-edit-{{$mov->idmovimiento}}">
		...
		<div class="modal-content" style="border-radius: 15px 15px 15px 15px;">
			<div class="modal-header" style="background:#333; border-radius: 10px 10px 0px 0px;">
				...
				aria-label="Close">
                 ...
                 <h5 class="modal-title" style="color:#fff;"><span class="label label" style="color:#fff; background:#019299; font-size:12px;">Editar Movimiento:</span><span style="color:#d9d6d8;"> {{ $mov->nombre}}</span></h5>
			</div>
			<div class="modal-body" style="background:#f8f8f8; border-radius: 0px 0px 10px 10px; ">
                     ...
                     @endif
                     	...
                     	{!!Form::model($mov,['method'=>'PATCH','route'=>['movimientos.ultimos.update',$mov->idmovimiento]])!!}
                        {{Form::token()}}
                    <div class="form-group">
                        <label for="nombre" style="color:#019299;">Nombre</label>
                        <input type="text" name="nombre" class="form-control" value="{{$mov->nombre}}" placeholder="Nombre..." style="color:#777;">
                    </div>
